...
GLfloat position_cloud_middle = 0.0f;
...
void update(int value) {
    ...
    if(position_cloud > 2.00f)
        position_cloud = -1.25f;

    position_cloud += speed;

    if(position_cloud_middle > 2.00f)
        position_cloud_middle = -1.25f;

    position_cloud_middle += speed;
...
//clouds
    ...
    glPushMatrix();
    glLoadIdentity();
      position_cloud +=0.02;  // 0.05
      glTranslatef(-position_cloud,0,0);
    cloud();
    glPopMatrix();

    glPushMatrix();
    glLoadIdentity();
      position_cloud_middle +=0.05;  // 0.05
      glTranslatef(-position_cloud_middle,0,0);
    cloud_middle();
    glPopMatrix();
...
int main(int argc, char** argv) {
   sndPlaySound("2022-12-25 02-01-22_trim.wav",SND_LOOP | SND_ASYNC);
   glutInit(&argc, argv);
